...
class Game(arcade.Window):
    ...
    def on_draw(self):
        ...
        if self.gameOver:
            if self.night:
                arcade.draw_text('YOU Lose', self.w//2-200, self.h//2, arcade.color.WHITE, DEFAULT_FONT_SIZE, width=400, align='center')
                arcade.draw_text('Press R for reset game', self.w//2-200, self.h//2-100, arcade.color.WHITE, DEFAULT_FONT_SIZE//2, width=400, align='center')
            else:
                arcade.draw_text('YOU Lose', self.w//2-200, self.h//2, arcade.color.BLACK, DEFAULT_FONT_SIZE, width=400, align='center')
            ...
            self.dino.draw()
            if self.night:
                arcade.draw_text('HI  %s  %s' %(str(self.dino.highScore).zfill(5), str(self.dino.score).zfill(5)), self.w-220, self.h-30, arcade.color.WHITE, DEFAULT_FONT_SIZE//2, width=400, align='left')
            else:
                arcade.draw_text('HI  %s  %s' %(str(self.dino.highScore).zfill(5), str(self.dino.score).zfill(5)), self.w-220, self.h-30, arcade.color.BLACK, DEFAULT_FONT_SIZE//2, width=400, align='left')
    def on_update(self, delta_time: float):
        ...
        if self.dino.time_startDown + 0.5 < self.time_now:
    ...
    def on_key_press(self, key, modifiers):
        if key==arcade.key.UP:
            ...
            self.dino.texture = arcade.load_texture('images/down0.png')
            self.dino.down = True
            self.dino.time_startDown = time.time()
